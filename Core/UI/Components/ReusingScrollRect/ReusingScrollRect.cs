...
public delegate void OnSetContentPosition();
...
public class ReusingScrollRect : ScrollRectInput
{
    ...
    //委托
    public OnSetContentPosition OnSetEvent;
    ...
    public int numberOfItemforLine = 0;
    ...
    public void SortVeriticalOrHoriticalLine(int countOfline, List<string> listStrs, ref List<List<string>> paramsList, ref int lines)
    {
        List<string> temp = new List<string>();
        lines = SortList(listStrs.Count, countOfline);
        foreach (var Item in listStrs)
        {
            temp.Add(Item);
            if (temp.Count == countOfline)
            {
            ...
            if (lines - 1 == paramsList.Count &&
                paramsList.Count * countOfline + temp.Count == listStrs.Count)
            {
                ...
                //print(temp2.Count + "---temp1.Count");
                paramsList.Add(temp2);
            }
        }

    }
    public void SortVeriticalOrHoriticalLine<T>(int countOfline, List<T> listStrs, ref List<List<T>> paramsList, ref int lines)
    {
    ...
    int SortList(int allNumber, int countOfLine)
    {
        int tempInt = 0;
        if (allNumber % countOfLine == 0)
        {
            tempInt = allNumber / countOfLine;
        }
        else
        {
            tempInt = (allNumber / countOfLine) + 1;
        }
        return tempInt;
    }
    #endregion
    ...
    {
        //print(m_items.Count + "---4");
        //print(m_indexList.Count + "---4");
