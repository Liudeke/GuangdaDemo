...
public class HotUpdateManager 
{
    ...
    static void CheckLocalVersion()
    {
        if (ApplicationManager.Instance.m_useAssetsBundle)
        {
            ...
            Debug.Log(StreamVersionContent+ "--StreamVersionContent");
            ab.Unload(true);

            //stream版本
            Dictionary<string, object> StreamVersion = (Dictionary<string, object>)FrameWork.Json.Deserialize(StreamVersionContent);

            Debug.Log(GetInt(StreamVersion[c_largeVersionKey])+ "--StreamVersion");
            Debug.Log(GetInt(m_versionConfig[c_largeVersionKey])+ "--m_versionConfig");
    ...
    static IEnumerator CheckVersion()
    {
        ...
        //www.assetBundle.Unload(true);//{"largeVersion":"{\"m_type\":2,\"m_content\":\"1\",\"m_enumType\":\"\"}","smallVerson":"{\"m_type\":2,\"m_content\":\"1\",\"m_enumType\":\"\"}","121":"{\"m_type\":7,\"m_content\":\"IsIgnoreTimeScale\",\"m_enumType\":\"AnimParamType\"}","11":"{\"m_type\":7,\"m_content\":\"OutQuad\",\"m_enumType\":\"InterpType\"}"}

        UpdateDateCallBack(HotUpdateStatusEnum.DownLoadingVersionFile, GetHotUpdateProgress(false, false, 1));

        Debug.Log("Version File :text: " + m_versionFileCache);
    ...
    static void Init()
    {
        ...
        foreach (var item in m_hotUpdateConfig)
        {
            //Debug.Log(item.Value.m_content+"--");
        }
        //使用测试下载地址
        if(isTest)
        {
            downLoadServicePath = m_hotUpdateConfig[c_testDownLoadPathKey].GetString();
            //Debug.Log("1232---" + downLoadServicePath);
        ...
        Debug.Log("=====>"+downLoadPath);//http://localhost:11575/Android/1.0/
        ...
        Debug.Log(s_versionFileDownLoadPath+ "--s_versionFileDownLoadPath");
        Debug.Log(s_Md5FileDownLoadPath + "--s_Md5FileDownLoadPath");
        Debug.Log(s_resourcesFileDownLoadPath + "--s_resourcesFileDownLoadPath");
    }
    ...
    public static string ReadVersionContent()
    {
        ...
        else
        {
            ...
            {
                AssetBundle ab = AssetBundle.LoadFromFile(PathTool.GetAbsolutePath(type,c_versionFileName + "." + AssetsBundleManager.c_AssetsBundlesExpandName));


                TextAsset text = (TextAsset)ab.mainAsset;
                dataJson = text.text;
                ab.Unload(true);
                //Debug.Log(PathTool.GetAbsolutePath(
                //    type,
                //    c_versionFileName + "." + AssetsBundleManager.c_AssetsBundlesExpandName));
